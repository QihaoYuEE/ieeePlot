function hSubPlot = ieeeOddSubplot(Dimension, RowRatio, ColRatio, AxesMargin, FigureSize, FigureOffset)
...
AxesW       = (SubPlotW)*RowRatio - MarginL - MarginR;
AxesH       = (SubPlotH)*ColRatio - MarginB - MarginT;

% The real subplot size
SubPlotW    = AxesW + MarginL + MarginR;
SubPlotH    = AxesH + MarginB + MarginT;
...
for NumCols     = 1:ncols
    for NumRows     = 1:nrows
        ...
        hSubPlot(NumRows,NumCols).Position     = [sum(SubPlotW(1:NumCols - 1)) + MarginL + OffsetL ...
            sum(SubPlotH(nrows:nrows - NumRows + 1)) + MarginB + OffsetB...
            AxesW(NumCols) AxesH(NumRows)];
    end
end
